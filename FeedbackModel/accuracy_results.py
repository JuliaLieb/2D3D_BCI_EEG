...
def xdf_to_mat_file(current_config_file, cwd):
    ...
    if is_feedback:
        ...
        save_to_mat(out_dir+'/erds_ses' + session + '_run' + run + '.mat', 'erds', erds_with_labels)
        save_to_mat(out_dir+'/lda_ses' + session + '_run' + run + '.mat', 'lda', lda_with_labels)
    else:
...
def calculate_accuracy(current_config_file):
    ...
    if int(run) == 1:
        ...
        print('Run ' + run + ': accuracy in %: ', accuracy * 100)
